...
export async function runTripAgent(input:string,currentTrip:TripPlan|null,history:Array<{role:string;text:string}>=[]):Promise<AgentResult>{
 ...
 if(route.data.intent==="revise_trip"&&currentTrip){
  ...
  if(route.data.intent==="new_trip"&&route.data.destination&&route.data.days&&route.data.days<=3){
   ...
   const dayResponses=await Promise.all(Array.from({length:route.data.days},(_,index)=>callModelJson<{day:TripDay}>({
    model:getDeepSeekModel(),maxTokens:1000,temperature:0,timeoutMs:38_000,
    messages:[{role:"system",content:`${dayPrompt}\n\n相关 Skills：\n${skillPrompt(selected)}`},{role:"user",content:JSON.stringify({userInput:input,destination:route.data.destination,dayNumber:index+1,totalDays:route.data.days,startDate:route.data.startDate,budget:route.data.budget,travelers:route.data.travelers,toolResults:prepared.context.toolResults})}]
   })));
   usage.push(...dayResponses.map(response=>response.usage));
   const budget=route.data.budget||0;const month=input.match(/\d{1,2}月/)?.[0];
   planned={message:`已按“${route.data.destination}${route.data.days}天”生成逐日路线，并结合同行人、预算和偏好控制节奏。`,trip:{id:"",title:`${route.data.destination}${route.data.days}天·可调整旅行计划`,destination:route.data.destination,dates:route.data.startDate?`${route.data.startDate} 起，共 ${route.data.days} 天`:month?`${month}，共 ${route.data.days} 天（具体日期待确认）`:`共 ${route.data.days} 天（日期待确认）`,travelers:route.data.travelers?`${route.data.travelers} 人`:"人数待确认",budget,version:1,days:dayResponses.map(response=>response.data.day),notices:["开放时间、票价与临时管制请在出发前再次核验。","本计划只提供查询与规划，不执行预订、下单或支付。"],assumptions:["未明确的住宿位置和往返大交通按待确认处理。"],sources:[],budgetBreakdown:budget?[{label:"住宿与往返交通",amount:Math.round(budget*.45)},{label:"活动与餐饮",amount:Math.round(budget*.4)},{label:"机动预算",amount:budget-Math.round(budget*.45)-Math.round(budget*.4)}]:[]}};
  }else{
