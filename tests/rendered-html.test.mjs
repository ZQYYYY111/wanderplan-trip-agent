...
test("renders the travel planning studio",async()=>{const response=await (await worker()).fetch(new Request("http://localhost/",{headers:{accept:"text/html"}}),env,ctx);assert.equal(response.status,200);const html=await response.text();assert.match(html,/漫游策/);assert.match(html,/京都 · 慢游秋日/);assert.match(html,/规划过程/);assert.match(html,/只提供查询与规划/);assert.doesNotMatch(html,/codex-preview|react-loading-skeleton/)});
...
test("rejects empty agent requests before calling the model",async()=>{const response=await (await worker()).fetch(new Request("http://localhost/api/agent",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({input:""})}),env,ctx);assert.equal(response.status,400);const body=await response.json();assert.match(body.error,/1–800/)});
